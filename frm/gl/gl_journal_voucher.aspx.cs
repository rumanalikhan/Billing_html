...
public partial class GL_Journal_Voucher : System.Web.UI.Page
{
            ...
            try
            {
                ...
                if (credit > 0)
                {
                    InsertVoucherEntry(conn, transaction, ref lineNumber, bookType, glCode,
                        slTypeId, actualSLCode, costCentreCode, billNumber, chequeNumber, "1", narration, credit, 1);
                    rowsSaved++;
                }
                ...
                {
                    InsertVoucherEntry(conn, transaction, ref lineNumber, bookType, glCode,
                        slTypeId, actualSLCode, costCentreCode, billNumber, chequeNumber, "2", narration, debit, 2);
                    rowsSaved++;
                }
            }
            catch (Exception ex)
    ...
    private void InsertIntoGLForms(OracleConnection conn)
    {
        ...
        cmd.Parameters.Add("compId", OracleDbType.Int32).Value = Convert.ToInt32(hfCompId.Value);
        cmd.Parameters.Add("logId", OracleDbType.Int32).Value = 0;
