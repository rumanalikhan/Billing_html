<%@ Page Language="C#" AutoEventWireup="true" CodeFile="voucher_status.aspx.cs" Inherits="voucher_status" %>
...
<head id="Head1" runat="server">
    <meta charset="UTF-8" />
    <title>Voucher Status Management</title>
    ...
    <style>
        ...
        .status-dropdown { width: 150px; padding: 8px 10px; font-size: 14px; border: 1px solid #bfbfbf; border-radius: 4px; background-color: white; font-weight: bold; }
        ...
        .btn-update { background: white; color: #2196F3; border: 1px solid #2196F3; }
        .btn-cancel { background: white; color: #ea4242; border: 1px solid #ea4242; }
        .btn-search:hover { background: #0f7c57; color: white; }
        .btn-update:hover { background: #2196F3; color: white; }
...
<body>
    <form id="form1" runat="server">
        ...
        <header>
            <div style="font-weight:bold; font-size:18px;">Voucher Status Management</div>
        ...
        <div class="container">
            ...
            <div class="left-panel">
                <h3>Voucher Status Management</h3>
                <ul>
                    <li>Search vouchers by Key or Type</li>
                    <li>View current status (Posted/Unposted)</li>
                    <li>Change status using dropdown</li>
                    <li>Click Update to save changes</li>
                    <li>Status updates are logged in audit trail</li>
                </ul>
                <div style="margin-top: 20px; padding: 10px; background: rgba(255,255,255,0.2); border-radius: 5px;">
                    <small><strong>Note:</strong> Changing status affects voucher posting in financial reports</small>
                </div>
            </div>
                ...
                <div class="card">
                    ...
                    <div class="grid-container">
                        <asp:GridView ID="gvResults" runat="server" CssClass="gridview-style" AutoGenerateColumns="False" 
                            OnRowCommand="gvResults_RowCommand" OnRowDataBound="gvResults_RowDataBound"
                            EmptyDataText="No vouchers found. Please try different search criteria.">
                            <Columns>
        ...
        <asp:TemplateField HeaderText="Current Status" ItemStyle-Width="120px">
            <ItemTemplate>
                <asp:Label ID="lblCurrentStatus" runat="server" CssClass="current-status" />
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="New Status" ItemStyle-Width="150px">
            <ItemTemplate>
                <asp:DropDownList ID="ddlNewStatus" runat="server" CssClass="status-dropdown">
                    <asp:ListItem Text="-- Select --" Value="" />
                    <asp:ListItem Text="Posted" Value="1" />
                    <asp:ListItem Text="Unposted" Value="0" />
                </asp:DropDownList>
            </ItemTemplate>
        </asp:TemplateField>
        <asp:TemplateField HeaderText="Action" ItemStyle-Width="80px">
            <ItemTemplate>
                <asp:LinkButton ID="lnkUpdate" runat="server" CommandName="UpdateStatus" 
                    CommandArgument='<%# Eval("VOUCHER_KEY") %>'
                    Text="Update" CssClass="action-link update-status" />
            </ItemTemplate>
        </asp:TemplateField>
    </Columns>
                        </asp:GridView>
