...
public partial class resid_info : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            // Disable RES_ID field initially
    ...
    protected void pcdInit()
    {
        ...
        // Enable all input fields for insert mode
                    ...
                    using (OracleCommand cmd = new OracleCommand(sql, con))
                    {
                        ...
                        using (OracleDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            else
                            {
                                ...
                                ddlCategory.ClearSelection();
                                if (ddlCategory.Items.Count > 0) ddlCategory.Items[0].Selected = true;

                                ddlNewCategory.ClearSelection();
                                if (ddlNewCategory.Items.Count > 0) ddlNewCategory.Items[0].Selected = true;

                                ddlPrcnt.ClearSelection();
                                if (ddlPrcnt.Items.Count > 0) ddlPrcnt.Items[0].Selected = true;

                                ddlBlock.ClearSelection();
                                if (ddlBlock.Items.Count > 0) ddlBlock.Items[0].Selected = true;

                                txtResId.Focus();
                                ShowStatusMessage("Registration Number not found", "error");
                            }
                        }
                    }
                }
...
//    {
//        string connMNT = WebConfigurationManager.ConnectionStrings["MyDbConnectionMNT"].ConnectionString;

//        try
//        {
//            string checkSql = "SELECT COUNT(*) FROM RESIDENTIAL_INFO WHERE RES_ID = :RES_ID";
//            int count = 0;

//            using (OracleConnection con = new OracleConnection(connMNT))
//            using (OracleCommand checkCmd = new OracleCommand(checkSql, con))
//            {
//                checkCmd.Parameters.Add(new OracleParameter("RES_ID", Convert.ToInt32(resId)));
//                con.Open();
...
//                        txtRegNo.Text = dr["RES_CODE"] != DBNull.Value ? dr["RES_CODE"].ToString() : "";
...
//                        {
//                            string rcatId = dr["RCAT_ID"].ToString();
//                            ListItem item = ddlCategory.Items.FindByValue(rcatId);
//                            if (item != null)
//                            {
//                                item.Selected = true;
//                            }
...
//                        {
//                            string precintId = dr["PRECENT_ID"].ToString();
//                            ListItem prcnt = ddlPrcnt.Items.FindByValue(precintId);
//                            if (prcnt != null)
//                            {
//                                prcnt.Selected = true;
//                            }
...
//                        {
//                            string blockId = dr["BLOCK_ID"].ToString();
//                            ListItem block = ddlBlock.Items.FindByValue(blockId);
//                            if (block != null)
//                            {
//                                block.Selected = true;
//                            }
...
//            // Load New Category
//            string sqlNewCat = @"SELECT CAT_ID, CAT_NM, CAT_COST 
//                                 FROM RES_CAT_MAINT_NEW 
//                                 WHERE RES_ID = :RES_ID";
...
//                        ListItem newItem = ddlNewCategory.Items.FindByValue(catId);
//                        if (newItem != null)
//                        {
...
//            // Load Electric ID
//            string sqlE = @"SELECT NVL(MAX(E.RESID),0)+1 RES_ID FROM BILLS.BIL_ELEC E 
//                            WHERE E.BGID=(SELECT B.BG_ID FROM BILL_GENERATE B WHERE B.IS_LOCKED='N')";

//            using (OracleConnection con = new OracleConnection(connMNT))
//            using (OracleCommand cmd = new OracleCommand(sqlE, con))
//            {
//                con.Open();
//                using (OracleDataReader dr = cmd.ExecuteReader())
//                {
//                    if (dr.Read())
//                    {
//                        txtResIdE.Text = dr["RES_ID"] != DBNull.Value ? dr["RES_ID"].ToString() : "";
//                    }
//                }
//            }
...
//    }
    ...
    protected bool LoadResidentialData(string resId)
    {
        ...
        try
        {
            ...
            // Load BARCODE from RESIDENTIAL_BARCODE table
            ...
            using (OracleCommand cmd = new OracleCommand(sqlBarcode, con))
            {
                ...
                using (OracleDataReader dr = cmd.ExecuteReader())
                {
                    ...
                    {
                        // Display the barcode in the Registration Number field
    ...
    protected void pcdBarcodeExists()
    {
        int mCount = 0;
        string btkNo = txtRegNo.Text.Trim();

        if (string.IsNullOrEmpty(btkNo))
            return;

        string connStr = WebConfigurationManager.ConnectionStrings["MyDbConnectionMNT"].ConnectionString;
        string sql = @"SELECT COUNT(*) CNTT FROM RESIDENTIAL_BARCODE WHERE BTYPE_ID=3 AND BARCODE=:btkNo";
        ...
        if (mCount == 0)
        {
            sql = @"SELECT NVL(MAX(RES_ID),0)+1 RES_ID FROM RESIDENTIAL_INFO";

            using (OracleConnection con = new OracleConnection(connStr))
            using (OracleCommand cmd = new OracleCommand(sql, con))
            {
                con.Open();
                txtResId.Text = cmd.ExecuteScalar().ToString();
            }

            sql = @"SELECT NVL(MAX(E.RESID),0)+1 RES_ID FROM BILLS.BIL_ELEC E 
                    WHERE E.BGID=(SELECT B.BG_ID FROM BILL_GENERATE B WHERE B.IS_LOCKED='N')";
            ...
            {
                con.Open();
                txtResIdE.Text = cmd.ExecuteScalar().ToString();
            }
        }
    }
    ...
    protected void InsertNewRecord()
    {
        ...
        string connMNT = WebConfigurationManager.ConnectionStrings["MyDbConnectionMNT"].ConnectionString;
        string connMain = WebConfigurationManager.ConnectionStrings["MyDbConnection"].ConnectionString;
        ...
        try
        {
            ...
            using (OracleCommand cmd = new OracleCommand(@"
                INSERT INTO RESIDENTIAL_INFO (
                    ...
                    COMP_ID, PRECENT_ID, BLOCK_ID, MAINT_CHARGES, STREET_ID, 
            ...
            using (OracleConnection con = new OracleConnection(connMNT))
            using (OracleCommand cmd = new OracleCommand(@"
            INSERT INTO RES_CAT_MAINT_NEW (
                RES_ID, CAT_ID, CAT_NM, CAT_COST
            ) VALUES (
                :RES_ID,:CAT_ID,:CAT_NM,:CAT_COST
            )", con))
            {
        ...
        try
        {
            ...
            using (OracleCommand cmd = new OracleCommand(@"
                UPDATE RESIDENTIAL_INFO SET
                    ...
                    RCAT_ID = :RCAT_ID,
                    PRECENT_ID = :PRECENT_ID,
                    BLOCK_ID = :BLOCK_ID,
                    MAINT_CHARGES = :MAINT_CHARGES,
                    STREET_ID = :STREET_ID
    ...
    protected void ShowStatusMessage(string message, string type)
    {
        ...
        string script = "setTimeout(function() { var elem = document.getElementById('" + statusMessage.ClientID + "'); if(elem) elem.style.display = 'none'; }, 3000);";
