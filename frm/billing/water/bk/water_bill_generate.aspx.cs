...
*/
                    ...
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT 'Bill Count='|| TO_CHAR(COUNT(*), 'FM999G999G999G999')|| ' and Bill Cost='|| TO_CHAR(NVL(ROUND(SUM(NET_PAYBLE), 0), 0),'FM999G999G999G999') FROM BILLS_WATER_TOBE",
                            con))
                    {
                        object cnt = cmdCnt.ExecuteScalar();
                        txtCrntBill.Text =
                    ...
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT 'Adv Cnt='||TO_CHAR(COUNT(*), 'FM999G999G999G999')||' and Cost='||TO_CHAR(NVL(ROUND(SUM(OPN_WALLET),0),0), 'FM999G999G999G999') FROM BILLS_WATER_TOBE WHERE OPN_WALLET>0",
                            con))
                    {
                        object cnt = cmdCnt.ExecuteScalar();
                        txtAdvance.Text =
                    ...
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT 'Fine Cnt='||TO_CHAR(COUNT(*), 'FM999G999G999G999')||' and Cost='||TO_CHAR(NVL(ROUND(SUM(FINE),0),0), 'FM999G999G999G999') FROM BILLS_WATER_TOBE WHERE FINE>0",
                            con))
                    {
                        object cnt = cmdCnt.ExecuteScalar();
                        txtFine.Text =
                    ...
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT 'Arer Cnt='||TO_CHAR(COUNT(*), 'FM999G999G999G999')||' and Cost='||TO_CHAR(NVL(ROUND(SUM(OPN_ARREARS),0),0), 'FM999G999G999G999') FROM BILLS_WATER_TOBE WHERE OPN_ARREARS>0",
                            con))
                    {
                        object cnt = cmdCnt.ExecuteScalar();
                        txtArrears.Text =
                    ...
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT 'Inst Cnt='||TO_CHAR(COUNT(*), 'FM999G999G999G999')||' and Bill Cost='||TO_CHAR(NVL(ROUND(SUM(ARR_INSTALLMENT),0),0), 'FM999G999G999G999') FROM BILLS_WATER_TOBE WHERE ARR_INSTALLMENT>0",
                            con))
                    {
                        object cnt = cmdCnt.ExecuteScalar();
                        txtInst.Text =
