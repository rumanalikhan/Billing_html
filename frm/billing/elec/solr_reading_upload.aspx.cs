...
public partial class solr_reading_upload : System.Web.UI.Page
{
    ...
    protected void btnUpload_Click(object sender, EventArgs e)
    {
            ...
            using (OracleConnection con = new OracleConnection(connStr))
            {
                ...
                try
                {
                    using (OracleCommand cmdCnt =
                        new OracleCommand(
                            "SELECT COUNT(*) FROM MRC_SOLR_DMY",
                            con))
                    {
                ...
                if (M_RECORD_COUNT > 0)
                {
                    using (OracleCommand cmdTrn =
                        new OracleCommand("TRUNCATE TABLE MRC_SOLR_DMY", con))
                    {
        ...
        try
        {
            using (OracleConnection con = new OracleConnection(connStr))
            {
                ...
                List<string> readPrvListK1 = new List<string>();
                List<string> readCrntListK1 = new List<string>();
                List<string> readPrvListK2 = new List<string>();
                List<string> readCrntListK2 = new List<string>();
                ...
                using (StreamReader sr = new StreamReader(fuCsv.FileContent))
                {
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        readPrvListK1.Add(cols[1].Trim());
                        readCrntListK1.Add(cols[2].Trim());
                        readPrvListK2.Add(cols[3].Trim());
                        readCrntListK2.Add(cols[4].Trim());
                ...
                if (rowCount > 0)
                {
                    using (OracleCommand cmd = new OracleCommand(
                        "INSERT INTO MRC_SOLR_DMY (BARCODE, K1P, K1C, K2P, K2C, MRDT) VALUES (:1, :2, :3, :4, :5, :6)",
                        con))
                    {
