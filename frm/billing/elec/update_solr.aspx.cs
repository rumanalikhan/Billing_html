...
public partial class update_solr : System.Web.UI.Page
{
    ...
    protected void pcdFetchRecord(object sender, EventArgs e)
    {
        ...
        string sql = @"SELECT 
                        ...
                        READ_PRV_SLR, READ_CURR_SLR, READ_DIFF_SLR, UNITS_SLR, UNIT_RATE_SLR, 
                        UNIT_AMOUNT_SLR, NET_UNITS_SLR_NET, FIXED_RATES_NET, NET_AMNT_SLR_NET, 
                        ALL_K_TOT_NET, PRE_AMT, Q1, Q2, Q3, Q4, TOT_PAYABLE_AMT_NET, 
                        ADVPAYMNTNET, ARREARSNET, INSTAMTNET, FINECHRGSNET, DCCHRGSNET, ONMCHRDSNET, BILAMNTBDDT, BILAMNTLP, BILAMNTADDT 
                    FROM BIL_ELEC
        ...
        using (OracleCommand cmd = new OracleCommand(sql, con))
        {
            ...
            using (OracleDataReader dr = cmd.ExecuteReader())
            {
                if (dr.Read())
                {
                    ...
                    txtK2From.Text = dr["READ_PRV_SLR"].ToString();
                    txtK2To.Text = dr["READ_CURR_SLR"].ToString();
                    txtSlDiff.Text = dr["READ_DIFF_SLR"].ToString();
                    txtUnitAdj.Text = dr["UNITS_SLR"].ToString();
                    txtAdjRate.Text = dr["UNIT_RATE_SLR"].ToString();
                    txtK2Amt.Text = Convert.ToDecimal(dr["UNIT_AMOUNT_SLR"]).ToString("N0");

                    txtFixedRate.Text = dr["FIXED_RATES_NET"].ToString();
                    txtNetUnit.Text = dr["NET_UNITS_SLR_NET"].ToString();
                    txtUnitPurchase.Text = dr["FIXED_RATES_NET"].ToString();
                    txtK3Amt.Text = Convert.ToDecimal(dr["NET_AMNT_SLR_NET"]).ToString("N0");

                    txtTltAmt.Text = dr["ALL_K_TOT_NET"].ToString();
                    txtPrevBal.Text = dr["PRE_AMT"].ToString();
                    txtQtr1.Text = dr["Q1"].ToString();
                    txtQtr2.Text = dr["Q2"].ToString();
                    txtQtr3.Text = dr["Q3"].ToString();
                    txtQtr4.Text = dr["Q4"].ToString();
                    txtTotAmt.Text = Convert.ToDecimal(dr["TOT_PAYABLE_AMT_NET"]).ToString("N0");
    ...
    protected void pcdReCalculateBill()
    {
        ...
        decimal K2From= ToDecimal(txtK2From.Text);
        decimal K2To= ToDecimal(txtK2To.Text);
        decimal SlDiff= ToDecimal(txtSlDiff.Text);
        decimal UnitAdj= ToDecimal(txtUnitAdj.Text);
        decimal AdjRate= ToDecimal(txtAdjRate.Text);
        decimal K2Amt = ToDecimal(txtK2Amt.Text);

        SlDiff = ((K2To - K2From) * MFactor);
        UnitAdj=Math.Min(mDiff, SlDiff);
        AdjRate = Math.Round((UnitRate * 0.81m), 2);
        K2Amt = Math.Round((UnitAdj * AdjRate), 0);

        txtSlDiff.Text = SlDiff.ToString();
        txtUnitAdj.Text = UnitAdj.ToString();
        txtAdjRate.Text = AdjRate.ToString();
        txtK2Amt.Text = K2Amt.ToString("N0");

        decimal NetUnit=ToDecimal(txtNetUnit.Text);
        decimal UnitPurchase=ToDecimal(txtUnitPurchase.Text);
        decimal K3Amt = ToDecimal(txtK3Amt.Text);

        NetUnit = Math.Round(Math.Max(0, mDiff >= (0.4m * SlDiff) ? (SlDiff - mDiff) : (0.6m * SlDiff)), 0);
        K3Amt = Math.Round((UnitPurchase * NetUnit),0);

        txtNetUnit.Text = NetUnit.ToString();
        txtK3Amt.Text = K3Amt.ToString("N0");

        decimal TltAmt = ToDecimal(txtTltAmt.Text);
        TltAmt = (BillCost - (K2Amt + K3Amt));
        txtTltAmt.Text = TltAmt.ToString("N0");

        decimal PrevBal=ToDecimal(txtPrevBal.Text);
        decimal Qtr1=ToDecimal(txtQtr1.Text);
        decimal Qtr2=ToDecimal(txtQtr2.Text);
        decimal Qtr3=ToDecimal(txtQtr3.Text);
        decimal Qtr4=ToDecimal(txtQtr4.Text);
        decimal TotAmt = ToDecimal(txtTotAmt.Text);

        txtTotAmt.Text = Math.Round((PrevBal + TltAmt), 0).ToString("N0");
        ...
        BillAmt=ToDecimal(txtBillAmt.Text);
        LatePayment=ToDecimal(txtLatePayment.Text);
        AfterDue=ToDecimal(txtAfterDue.Text);

        if (TotAmt < 0)
        {
            if (Instalment > 0)
            {
                BillAmt = Math.Round(((ONMCharges + DcCharges + Instalment + Fine) - Advance), 0);
            }
            else
            {
                BillAmt = Math.Round(((ONMCharges + DcCharges + Arrear + Fine) - Advance), 0);
            }
            ...
            {
                LatePayment = Math.Round((ONMCharges * 0.1m), 0);
            }
        ...
        else
        {
            if (Instalment > 0)
            {
                BillAmt = Math.Round(((ONMCharges + DcCharges + Instalment + Fine + TotAmt) - Advance), 0);
            }
            else
            {
                BillAmt = Math.Round(((ONMCharges + DcCharges + Arrear + Fine + TotAmt) - Advance), 0);
            }
